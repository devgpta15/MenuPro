...
const resolveImageUrl = (path, fallbackIndex = 0) => {
  if (!path) return `${API_ORIGIN}${dishPhotos[fallbackIndex % dishPhotos.length]}`;
...
export default function RestaurantDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const menuData = (r.foodItems || []).map((f, index) => ({
          id: f.foodItemId,
          ...
          imageUrl: resolveImageUrl(f.imagePath || f.imageUrl || f.photoUrl, index),
        }));

        const restaurantId = r.restaurantId ?? r.id;
        const restaurantImage = resolveImageUrl(r.imagePath, 0);
        const galleryPhotos = [
          restaurantImage,
          ...menuData.map((item) => item.imageUrl),
        ].filter(Boolean);
  ...
  const imageUrl = resolveImageUrl(restaurant.imagePath, 0);
    ...
    <div className="restaurant-details-page">
      ...
      <div className="restaurant-header">
        <img
          ...
          alt={restaurant.name}
          onError={(e) => {
            e.currentTarget.src = `${API_ORIGIN}${dishPhotos[0]}`;
          }}
        />
